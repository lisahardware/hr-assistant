...
C4Component
    ...
    Container_Boundary(app, "Flask Web App — hr-app") {
        Component(routes, "API Routes", "Flask / main.py", "HTTP endpoints bridging the browser to the agent pipeline. POST /api/request delegates to CoordinatorAgent; GET /api/status checks MCP availability and active model; POST /api/download and /api/save-onedrive handle document delivery. GET /api/compliance-status returns last-refreshed date and chunk count from VectorStore; POST /api/compliance-refresh triggers ComplianceRefresh on demand.")
        ...
        Component(vectorStore, "VectorStore", "Python / tools/vector_store.py", "Wraps two ChromaDB collections using the default sentence-transformer embedding function (fully local). Indexes .docx templates chunked by section heading. Exposes retrieve_templates() and retrieve_compliance() for semantic search, and get_compliance_last_refreshed() which derives the most recent refresh date from chunk metadata.")
        Component(complianceRefresh, "ComplianceRefresh", "Python / tools/compliance_refresh.py", "Fetches the latest MD, DC, and VA labor law references from public sources (eCFR, IRS, state sites) and indexes them into the compliance_sources ChromaDB collection via VectorStore. Called automatically at startup when the collection is empty, and on demand via POST /api/compliance-refresh. Uses URL-hashed chunk IDs to prevent per-state ID collisions when multiple sources are indexed.")
    ...
    Rel(routes, complianceRefresh, "refresh_all() — on demand and on first startup")
    Rel(complianceRefresh, vectorStore, "index_compliance_text() — writes fetched sources")
